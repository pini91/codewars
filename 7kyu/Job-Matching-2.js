...
let candidates = [{
  ...
  //And a job might look like this:
let job1 = { equityMax: 1, locations: ['Los Angeles', 'New York'] }

function match(job, candidates) {
    return candidates.filter(candidate=> (job.equityMax>0||!candidate.desiresEquity)&& job.locations.some(el=> candidate.currentLocation===el || candidate.desiredLocations.includes(el)))
    // job.equity>0 || theres no desire equity from the candidates
    //if job.equity> 0 then it doesnt matter if the candicates wants or not equity 
    //or if the candidate doesnt want an equirty
}
