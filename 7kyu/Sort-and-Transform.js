...
function sortTransform(a){
    ...
    let first = [a[0],a[1],a[a.length-2],a[a.length-1]]

    a.sort((a,b)=>a-b)

    let second =[a[0],a[1],a[a.length-2],a[a.length-1]]

    a.sort((a,b)=>b-a)

    let third =[a[0],a[1],a[a.length-2],a[a.length-1]]

    let four = `${first.map(el=>String.fromCharCode(el)).join('')}-${second.map(el=>String.fromCharCode(el)).join('')}-${third.map(el=>String.fromCharCode(el)).join('')}-${third.map(el=>String.fromCharCode(el)).sort().join('')}`
