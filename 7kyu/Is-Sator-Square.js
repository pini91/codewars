...
function isSatorSquare(tablet) {
    ...
    // Compare each position with its corresponding positions in all directions
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < n; j++) {
            //Compare with reverse position
            if (tablet[i][j] !== tablet[n-1-i][n-1-j]) {
                return false;
            }
           //Compare with transposed position
...
console.log(isSatorSquare(tests))