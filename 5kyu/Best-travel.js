...
function chooseBestSum(t, k, ls) {
    ...
    function getCombinations(arr, comboLength, startIndex, currentCombo) {

        if (currentCombo.length === comboLength) {
        const currentSum = currentCombo.reduce((acc, curr) => acc + curr, 0);
 
        if (currentSum <= t && currentSum > (bestSum || 0)) {
            bestSum = currentSum;
        }
        return;
        }
        ...
        getCombinations(arr, comboLength, i + 1, currentCombo);
        currentCombo.pop(); // Backtrack
        }
    }

    getCombinations(ls, k, 0, []);
    return bestSum;
}
